...
class HospitalOcrNode(Node):
    ...
    def _process_request(self, context: RequestContext, frames: list[np.ndarray]) -> dict[str, Any]:
        ...
        # Mission-target-first OCR rule: the command already tells us which patient
        # should be in front of this AMR.  Verify that requested patient when BOTH
        # the name and birth date are approximately consistent with the OCR evidence.
        # Canonical expected values are then returned, so a minor OCR typo does not
        # block the fixed hospital demo.
        name_ok = expected_name_sim >= 0.40
        birth_ok = expected_birth_sim >= 0.55
        strong_name_with_partial_birth = expected_name_sim >= 0.66 and expected_birth_sim >= 0.35
        strong_birth_with_partial_name = expected_birth_sim >= 0.88 and expected_name_sim >= 0.25
        verified = bool(
            expected_candidate
            and enough_frames
            and (
                (name_ok and birth_ok)
                or strong_name_with_partial_birth
                or strong_birth_with_partial_name
            )
        )
