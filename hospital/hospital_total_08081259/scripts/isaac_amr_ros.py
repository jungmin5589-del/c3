#!/usr/bin/env python3
"""Isaac Sim standalone app for hospital AMRs, elevator, ROS images, OCR, and ArUco cards.
...
- The mission manager uses OCR only for identity and paired ArUco results for bed-centre alignment.
- The legacy Isaac AutoApproach path remains available for compatibility but is not used by mission launch.
"""
...
class AutomaticElevatorSequence:
    ...
    def start(self, payload: dict[str, Any]) -> None:
        ...
        # DOWN return starts only after Nav2 has already stopped at the fixed 2F elevator goal.
        # Keep the 2F door CLOSED while rotating RIGHT exactly 90 deg.  Only after the
        # rotation is fully complete do we open the door and drive to world Y=25.91.
        if state not in {"IDLE_2_CLOSED", "IDLE_2_OPEN"}:
            self.fail(f"elevator is not ready on 2F: {state}")
            return
        # Nav2 is already stopped here.  Make sure a stale elevator passive-physics
        # flag cannot swallow the rotation command, then face the cabin directly.
        self.controller.set_external_physics_mode(False)
        self.controller.halt()
        self.previous_yaw = world_yaw(self.controller.base_prim)
        self.rotation_progress = 0.0
        self.rotation_target_yaw = math.pi * 0.5  # world +Y = elevator cabin direction
...
def print_help() -> None:
    ...
    print("Mission     : patient_transport_manager.py selects patient -> OCR verify -> ArUco pair align -> patient-specific forward")
...
def main() -> int:
    ...
    print(
        f"[ARUCO READY] separate_cards={len(aruco_cards)} "
        "expected=6 original_nameplates=UNCHANGED"
    )
    controllers = [
