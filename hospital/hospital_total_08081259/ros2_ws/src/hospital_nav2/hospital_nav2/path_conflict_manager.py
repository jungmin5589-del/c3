...
class PathConflictManager(Node):
    def __init__(self) -> None:
        ...
        self.declare_parameter("path_stale_sec", 30.0)
        ...
        self.get_logger().info(
            ...
            f"hold_trigger={self.hold_trigger_distance:.1f}m, release_delay={self.release_delay:.1f}s, special-priority=ON"
        )
    ...
    def _tick(self) -> None:
        ...
        if special:
            ...
            self._publish_status(
                "SPECIAL_PRIORITY",
                f"{winner.upper()} non-Nav special motion has absolute priority; {loser.upper()} Nav waits",
                winner=winner,
                loser=loser,
            )
            return
        if self.special_owner is not None:
            ...
            self._set_pause("amr1", False)
            self._set_pause("amr2", False)
            self._publish_status("FREE", f"{previous.upper()} special motion finished; paused Nav resumed")
            return
        ...
        if now - a.path_stamp > self.path_stale_sec or now - b.path_stamp > self.path_stale_sec:
            return
