...
def generate_launch_description() -> LaunchDescription:
    ...
    aruco_node = Node(
        ...
        name="amr2_aruco_pair",
        output="screen",
        parameters=[{
            "amr_id": "amr2",
            "image_topic": "/amr2/camera/front/color/image_raw",
            "result_topic": "/amr2/aruco/result",
            "debug_image_topic": "/amr2/aruco/debug_image",
    ...
    mission_manager = TimerAction(
        ...
        actions=[
            ExecuteProcess(
                cmd=["python3", PathJoinSubstitution([project_root, "patient_transport_manager.py"]), patient, "--amr", "amr2"],
                output="screen",
            )
        ],
    )
