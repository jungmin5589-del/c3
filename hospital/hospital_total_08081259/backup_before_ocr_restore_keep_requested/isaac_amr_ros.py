...
class AMRController:
    ...
    def ensure_magnet_lock(self) -> bool:
        """Apply the same bed-lock action as C, but only when the joint is missing."""
        ...
        # A PhysX handoff can remove the joint while the cached lock flag remains True.
        # Clear only that stale flag, then run the normal C/LOCK path unchanged.
    ...
    def update(
        ...
        # With a bed attached, directly forcing base-link linear velocity while also
        # driving all four wheel joints makes the two physics controls fight each
        # other.  That conflict is the forward/reverse judder.  Loaded motion uses
        # the already-strengthened wheel drives only; unloaded motion is unchanged.
        if not self.magnet.locked:
